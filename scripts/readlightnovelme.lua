...
function search(searchQuery)
    ...
    local request = lib:getRequestBuilder():url('https://www.readlightnovel.me/search/autocomplete')
    :addHeader("content-length", tostring(contentLength+2))
	:addHeader("content-type", "application/x-www-form-urlencoded; charset=UTF-8")
    :addHeader("origin", "https://www.readlightnovel.me")
    :addHeader("referer", "https://www.readlightnovel.me/hub141222")
    :addHeader("X-Requested-With", "XMLHttpRequest")
    :post(body)
    :build()
    local result = lib:executeRequest(request, 'https://www.readlightnovel.me')
    local decoded = lib:decodeUnicode(result:toString())

    local chapterList = lib:jsoupParseHtml(decoded, 'https://www.readlightnovel.me'):select('li')
